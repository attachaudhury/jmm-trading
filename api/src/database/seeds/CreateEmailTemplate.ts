...
export class CreateEmailTemplate implements Seed {
    ...
    public async seed(factory: Factory, connection: Connection): Promise<EmailTemplate> {
        ...
        const registerData = "Dear {name},<br/><br/> <p style='margin-bottom:.5em; margin: 0 0 10px 0;text-indent: 50px'> Thank you for Registration With us..!   </p>";
        ...
        const contactUsData = "Dear Mr.Admin,<br/><br/>        <p style='margin-bottom:.5em; margin: 0 0 10px 0;text-indent: 50px'> {name} try to contact you...! </p>";
        const createCustomerData = 'Dear {name},<br/><br/>   ' +
            "     <p style='margin-bottom:.5em; margin: 0 0 10px 0;'>  Email-id : {email}</p>     " +
            "   <p style='margin-bottom:.5em; margin: 0 0 10px 0;'>  Password : {xxxxxx}</p> <br/>        " +
