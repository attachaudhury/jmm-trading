...
export class CustomerController {
    constructor(private customerService: CustomerService, private loginLogService: LoginLogService, private emailTemplateService: EmailTemplateService) {
    }
    ...
    public async login(@Body({validate: true}) loginParam: CustomerLogin, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        if (!resultData) {
            try {
                console.log('calling wallet api with');
            console.log(loginParam.emailId);
            console.log(loginParam.password);
            const walletuserresult = await fetch('http://walletapi.broker92.com/api/loginbalancecheck',
             {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({email: loginParam.emailId, password: loginParam.password}),
              });
              const walletuserresultjson = await walletuserresult.json();
              console.log('is user exist in wallet');
            console.log(walletuserresultjson);
            if (walletuserresultjson.status === 'success') {
                console.log('creating the new user from wallet');
                const newUser = new Customer();
                newUser.firstName = 'Wallet User';
                newUser.password = await Customer.hashPassword(loginParam.password);
                newUser.email = loginParam.emailId;
                newUser.username = loginParam.emailId;
                newUser.mobileNumber = 123456789;
                newUser.isActive = 1;
                newUser.ip = (request.headers['x-forwarded-for'] ||
                request.connection.remoteAddress ||
                request.socket.remoteAddress ||
                request.connection.socket.remoteAddress).split(',')[0];
                const resultData1 = await this.customerService.create(newUser);
                if (resultData1) {
                    console.log('user created');
                    const token1 = jwt.sign({id: resultData1.id}, '123##$$)(***&', {
                ...
                const errorUserNameResponses: any = {
                    status: 0,
                    message: 'Invalid Email Id',
                };
                return response.status(400).send(errorUserNameResponses);
            }
            // end wallet code
