...
export class UserController {
    ...
    @Put('/update-user/:id')
    @Authorized()
    public async updateUser(@Param('id') id: number, @Body({ validate: true }) createParam: updateUserRequest, @Res() response: any): Promise<any> {        const userGroupExistWhereCondition = [
            {
                name: 'id',
                value: createParam.userGroupId,
            },
