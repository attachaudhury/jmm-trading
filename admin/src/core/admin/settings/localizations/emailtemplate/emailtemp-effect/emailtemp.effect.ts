...
export class EmailTempEffect {
    ...
    // NEW EMAIL_TEMP
    @Effect()
    doAddEmailTemp$: Observable<Action> = this.action$
        .pipe(
            ofType(actions.ActionTypes.DO_NEW_EMAIL_TEMP_ACTION),
            map((action: actions.DoNewEmailTempAction) => action.payload),
            switchMap((state) => {
                return this.service.addEmailtemp(state)
                    .pipe(
                        switchMap((role) => [
                            new actions.DoNewEmailTempSuccessAction(role),
                        ]),
    ...
    @Effect()
    doUpdateEmailTemp$: Observable<Action> = this.action$
        .pipe(
            ofType(actions.ActionTypes.DO_UPDATE_EMAIL_TEMP_ACTION),
            map((action: actions.DoUpdateEmailTempAction) => action.payload),
            switchMap((state) => {

                return this.service.updateEmailTemp(state)
                    .pipe(
                        switchMap((user) => {
                            return [
                                new actions.DoUpdateEmailTempSuccessAction(user),
    ...
    // EMAIL TEMP LIST
    @Effect()
    doemailtemplists$: Observable<Action> = this.action$
        .pipe(
            ofType(actions.ActionTypes.GET_EMAIL_TEMP_LIST_ACTION),
            map((action: actions.DoEmailTemplistAction) => action.payload),
            switchMap((state) => {
                return this.service.emailtemplist(state)
                    .pipe(
                        switchMap((user) => [
                            new actions.DoEmailTemplistSuccessAction(user),
                        ]),
    ...
    @Effect()
    doemailtemppagination$: Observable<Action> = this.action$
        .pipe(
            ofType(actions.ActionTypes.GET_EMAIL_TEMP_COUNT_ACTION),
            map((action: actions.DoEmailTempPaginationAction) => action.payload),
            switchMap((state) => {
                return this.service.emailtempPagiantion(state)
                    .pipe(
                        switchMap((user) => [
                            new actions.DoEmailTempPaginationSuccessAction(user),
                        ]),
                        catchError(error => of(new actions.DoEmailTempPaginationFailAction(error)))
                    );
            })
        );
// EMAIL_TEMP DELETE
    @Effect()
    doEmailtempDelete$: Observable<Action> = this.action$
        .pipe(
            ...
            switchMap((state) => {
                ...
                return this.service.deleteEmailtemp(state, emailTemplateId)
                    .pipe(
                        switchMap((user) => [
                            new actions.DoEmailTempDeleteSuccessAction((user)),
                        ]),
