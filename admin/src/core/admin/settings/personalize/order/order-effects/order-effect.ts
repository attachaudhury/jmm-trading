...
export class PersonalizeOrderEffect {
    ...
    constructor(private action$: Actions, private service: PerSonalizeOrderService) {
    }
    ...
    map((action: actions.DoNewOrderSettingAction) => action.payload),
            switchMap((state) => {
                return this.service.createOrder(state)
                    .pipe(
                        switchMap((user) => [
                            new actions.DoNewOrderSettingSuccessAction(user),
                        ]),
            ...
            switchMap(() => {
                return this.service.getOrder()
                    .pipe(
                        switchMap((user) => {
                            return [
                                new actions.DoGetOrderSettingSuccessAction((user)),
                            ];
                        }),
