...
import {PersonalizeOrderState, PersonalizeOrderRecordState} from './order-state';
...
export function reducer(state = initialState, {type, payload}: any): PersonalizeOrderState {
    if (!type) {
        return state;
    }
...
export const getnewPersonalizeOrder = (state: PersonalizeOrderState) => state.newPersonalizeOrder;
export const getPersonalizeOrder = (state: PersonalizeOrderState) => state.getPersonalizeOrder;
